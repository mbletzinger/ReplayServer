...
public class InterpolateTestData {
	 ...
	 *            The expected matrix.
	 * @param actual
	 *            The matrix generated from the code under test.
	 */
	public final void compareList2Doubles(final DoubleMatrixI expected,
		...
		Assert.assertEquals(asizes[0], esizes[0]);
		Assert.assertEquals(asizes[1], esizes[1]);
		final double errorTolerance = 0.01;
		for (int r = 0;r < esizes[0];r++) {
			for (int c = 0;c < esizes[1];c++) {
				if (Double.isNaN(expected.value(r, c))) {
					Assert.assertTrue(Double.isNaN(actual.value(r, c)));
				} else {
					Assert.assertEquals(actual.value(r, c),
							expected.value(r, c), errorTolerance);
				}
			}
		}
	}
